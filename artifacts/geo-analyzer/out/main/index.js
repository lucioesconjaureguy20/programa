...
let currentRegion = store.get("region", null);
const logsDir = path.join(electron.app.getPath("userData"), "prediction-logs");
...
function createSelectorWindow() {
  ...
  selectorWindow = new electron.BrowserWindow({
    x: 0,
    y: 0,
    width,
    height,
...
function savePredictionLog(prediction, imageBase64) {
  ...
  const logFile = path.join(logsDir, `predictions-${(/* @__PURE__ */ new Date()).toISOString().split("T")[0]}.jsonl`);
...
async function runCaptureLoop() {
  ...
  const tick = async () => {
    ...
    try {
      ...
      mainWindow?.webContents.send("prediction", { result, timestamp: (/* @__PURE__ */ new Date()).toISOString() });
      mainWindow?.webContents.send("capture-status", "capturing");
    } catch (err) {
  ...
  await tick();
  captureInterval = setInterval(tick, 5e3);
}
...
electron.ipcMain.handle("save-settings", (_e, { apiKey, region }) => {
  if (apiKey !== void 0) store.set("apiKey", apiKey);
  if (region !== void 0) {
    store.set("region", region);
    currentRegion = region;
  }
});
electron.ipcMain.handle("start-capture", async () => {
  if (!currentRegion) {
...
electron.ipcMain.handle("stop-capture", () => {
  stopCaptureLoop();
});
electron.ipcMain.handle("open-selector", () => {
  if (selectorWindow) {
...
electron.ipcMain.handle("region-selected", (_e, region) => {
  currentRegion = region;
  store.set("region", region);
...
electron.ipcMain.handle("close-selector", () => {
  selectorWindow?.close();
  mainWindow?.focus();
});
electron.ipcMain.handle("get-logs", () => {
  try {
    const logFile = path.join(logsDir, `predictions-${(/* @__PURE__ */ new Date()).toISOString().split("T")[0]}.jsonl`);
...
electron.ipcMain.handle("get-logs-path", () => logsDir);
electron.ipcMain.handle("open-logs-folder", () => {
  electron.shell.openPath(logsDir);
});
electron.ipcMain.handle("minimize-window", () => mainWindow?.minimize());
electron.ipcMain.handle("close-window", () => electron.app.quit());
electron.ipcMain.handle("capture-single", async () => {
